...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if fileExists(t, req, "install.sh") {
		t.Fatal("install.sh should not be created in dry-run")
	}
	...
	if !strings.Contains(out, "install.sh") {
		t.Fatalf("expected installer path install.sh in preview, got:\n%s", resp.Combined)
	}
}
```